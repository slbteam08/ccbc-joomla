...
foreach ($list as $item) : 
    // ↓↓↓↓↓↓↓↓↓↓↓↓ 【隱藏沒有文章的 Cat】↓↓↓↓↓↓↓↓↓↓↓↓
   $db = JFactory::getDbo();
if($db->setQuery("SELECT COUNT(*) FROM #__content a LEFT JOIN #__categories b ON a.catid=b.id WHERE b.lft >= {$item->lft} AND b.rgt <= {$item->rgt} AND a.state=1")->loadResult() ==0) continue;
    // ↑↑↑↑↑↑↑↑↑↑↑↑ 【唯一修改，僅2行代碼】↑↑↑↑↑↑↑↑↑↑↑↑
  ?>
	<?php
