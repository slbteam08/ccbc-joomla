<?php
/**
 * Sermon Blog Item - Keeps all original params/config
 */
...
// === CUSTOM FIELDS LOGIC ===
$speaker = ''; $sermontitle = ''; $youtubeUrl = ''; $coverImage = ''; $mp4Url = ''; $mp3Url = '';
if (!empty($this->item->jcfields)) {
    foreach ($this->item->jcfields as $field) {
        switch ($field->name) {
            case 'speaker': $speaker = $field->value ?? ''; break;
           //case 'sermon-title': $sermontitle = $field->value ?? ''; break;
            case 'youtube-link': $youtubeUrl = $field->value ?? ''; break;
            case 'cover-image': $coverImage = $field->value ?? ''; break;
...
$sermontitle = $sermontitle ?: $this->item->title;
...
<div class="item-content">
    ...
    <!-- === YOUR CUSTOM SERMON CONTENT (replaces introtext) === -->
    <?php if ($speaker || $fullYoutubeUrl || $fullCoverImageUrl) : ?>
    <div class="sermon">
        <div class="sermon-title">講員：<?php echo htmlspecialchars($speaker); ?></div>
        ...
        <a class="jcemediabox" href="<?php echo htmlspecialchars($fullYoutubeUrl); ?>" data-mediabox="youtube" data-mediabox-width="900" data-mediabox-height="506">
            <img src="<?php echo htmlspecialchars($fullCoverImageUrl); ?>" alt="<?php echo htmlspecialchars($sermontitle); ?>" class="sermon-thumb" />
        </a>
        <?php endif; ?>
        <?php if ($mp4Url || $mp3Url) : ?>
        <small>下載講道 <a href="<?php echo htmlspecialchars($mp4Url); ?>" target="_blank" rel="noopener noreferrer" >MP4</a> / <a href="<?php echo htmlspecialchars($mp3Url); ?>" target="_blank" rel="noopener noreferrer">MP3</a></small>
        <?php endif; ?>
    </div>
    <?php endif; ?>
