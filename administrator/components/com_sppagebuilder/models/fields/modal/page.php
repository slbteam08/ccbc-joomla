...
class JFormFieldModal_Page extends FormField
{
	...
	protected function getInput()
	{
		...
		if ($JoomlaVersion >= 4)
		{
			...
			$wa->addInlineScript(
				"
				window.jSelectPage_" . $this->id . " = function (id, title, catid, object, url, language, extensionView) {
					window.processModalSelect('Page', '" . $this->id . "', id, title, catid, object, url, language);
					const collectionField = document.querySelector('.control-group:has(#jform_request_collection_id)');
					document.querySelector('#jform_request_extension_view').value = extensionView;
					collectionField.dispatchEvent(new Event('change'));
				}",
				[],
				['type' => 'module']
			);
			...
			if ((int) $this->value > 0)
			{
				...
				try
				{
					$item = $db->loadObject();
					$title = $item->title;
					$extensionView = $item->extension_view;
				}
			...
			$html .= '<input type="hidden" id="jform_request_extension_view" value="' . $extensionView . '" >';
			$html .= '<button'
