 ...
 * @author mafung [https://www.knockingcreative.com](https://www.knockingcreative.com)
 * @copyright Copyright (c) 2026 mafung
...
class SppagebuilderAddonSermon_addon extends SppagebuilderAddons 
{
		...
		if (count((array) $items)) {
			...
			foreach ($items as $key => $item) {
				...
				if ($show_custom_field) {
					...
					if ((float) JVERSION >= 4) {
                            JLoader::registerNamespace('Joomla\CMS\Fields', JPATH_ADMINISTRATOR . '/components/com_fields/src');
              ...
              /* $custom_fields = FieldsHelper::getFields('com_content.article', $item);
                    ...
                    $speaker = ''; $youtubeUrl = ''; $coverImage = ''; $mp4Url = ''; $mp3Url = '';
                    foreach ($custom_fields as $field) {
                        switch ($field->name) {
                            case 'speaker':
                                $speaker = $field->rawvalue ?? $field->value ?? '';
                                break;
                            case 'youtube-link':
                                $youtubeUrl = $field->rawvalue ?? $field->value ?? '';
                                break;
                            case 'cover-image':
                                $coverImageJson = $field->rawvalue ?? $field->value ?? '';
                                $coverImageData = json_decode($coverImageJson, true);
                                $coverImage = $coverImageData['imagefile'] ?? '';
                                // Extract clean path (before #joomlaImage) and fix slashes
                                if ($coverImage) {
                                    $coverImage = explode('#joomlaImage', $coverImage)[0];
                                    $coverImage = str_replace('\\/', '/', $coverImage);
                                }
                                break;
                            case 'mp4-link':
                                $mp4Url = $field->rawvalue ?? $field->value ?? '';
                                break;
                            case 'mp3-link':
                                $mp3Url = $field->rawvalue ?? $field->value ?? '';
                                break;
                        }
                    }
                    $fullYoutubeUrl = $youtubeUrl ? 'https://www.youtube.com/watch?v=' . $youtubeUrl : '';
                    $fullCoverImageUrl = $coverImage ? JUri::root() . ltrim($coverImage, '/') : '';


               // After extracting fields...
                if ($speaker || $fullYoutubeUrl || $fullCoverImageUrl || $mp4Url || $mp3Url) {
                    $output .= '<div class="sermon">';
                    $output .= '<div class="sermon-title">講員：' . htmlspecialchars($speaker) . '</div>';
                    
                    if ($fullYoutubeUrl && $fullCoverImageUrl) {
                        $output .= '<a class="jcemediabox" href="' . htmlspecialchars($fullYoutubeUrl) . '" ';
                        $output .= 'data-mediabox="youtube" data-mediabox-width="900" data-mediabox-height="506">';
                        $output .= '<img src="' . htmlspecialchars($fullCoverImageUrl) . '" alt="Sermon" class="sermon-thumb" loading="lazy" />';
                        $output .= '</a>';
                    }
                    
                    if ($mp4Url || $mp3Url) {
                        $output .= '<small>下載講道 ';
                        if ($mp4Url) $output .= '<a href="' . htmlspecialchars($mp4Url) . '" target="_blank" rel="noopener">MP4</a>';
                        if ($mp3Url) $output .= ' / <a href="' . htmlspecialchars($mp3Url) . '" target="_blank" rel="noopener">MP3</a>';
                        $output .= '</small>';
                    }
