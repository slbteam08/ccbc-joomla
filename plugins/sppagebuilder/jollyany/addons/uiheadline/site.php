...
class SppagebuilderAddonUiHeadline extends SppagebuilderAddons {
	...
	public function render() {
		...
		$animated_texts = ( isset( $settings->ui_animated_list_item ) && $settings->ui_animated_list_item ) ? $settings->ui_animated_list_item : array();

		$heading_styles  = ( isset( $settings->heading_style ) && $settings->heading_style ) ? ' uk-' . $settings->heading_style : '';
		$heading_styles .= ( isset( $settings->headline_styles ) && $settings->headline_styles ) ? ' ' . $settings->headline_styles : 'zoom';
		...
		$output .= '<div class="ui-headline' . $zindex_cls . $general . '"' . $animation . '>';
		...
		$output .= '<' . $heading_selector . ' class="cd-headline' . $heading_styles . $font_weight . '">';

		$output .= ( $before_text ) ? '<span class="ui-headline uk-margin-small-right">' . $before_text . '</span>' : '';

		if ( count( (array) $animated_texts ) ) {
			$output .= '<span class="cd-words-wrapper">';
			foreach ( $animated_texts as $key => $animated_text ) {
				$title         = ( isset( $animated_text->title ) && $animated_text->title ) ? $animated_text->title : '';
				$visible_class = '';
				if ( $key == 0 ) {
					$visible_class = ' class="is-visible"';
				}
				$output .= '<b' . $visible_class . '>' . $title . '</b>';
			}
			$output .= '</span>';
		}
		$output .= ( $after_text ) ? '<span class="ui-headline uk-margin-small-left">' . $after_text . '</span>' : '';
	...
	public function css() {
		$settings         = $this->addon->settings;
		$heading_color    = ( isset( $settings->color ) && $settings->color ) ? ' color: ' . $settings->color . ';' : '';
		...
		$heading_styles   = ( isset( $settings->heading_style ) && $settings->heading_style ) ? $settings->heading_style : '';
		$heading_fontsize = ( isset( $settings->heading_fontsize ) && $settings->heading_fontsize ) ? 'font-size: ' . $settings->heading_fontsize . 'px; ' : '';
		$css              = '';

		$addon_id = '#sppb-addon-' . $this->addon->id;
		$css     .= "\n";
		$css     .= $addon_id . ' .cd-words-wrapper b {font-weight: inherit;}';
		$css     .= "\n";

		if ( $title_color ) {
			$css .= $addon_id . ' .ui-headline {' . $title_color . '}';
		}

		if ( ! empty( $heading_color ) ) {
			$css .= "\n";
			$css .= $addon_id . ' .ui-headline {' . $heading_color . '}';
			$css .= "\n";
		}
		if ( ! empty( $text_color ) ) {
			$css .= "\n";
			$css .= $addon_id . ' .cd-words-wrapper {' . $text_color . '}';
			$css .= "\n";
		}
		if ( ! empty( $heading_fontsize ) && empty( $heading_styles ) ) {
			$css .= "\n";
			$css .= $addon_id . ' .cd-headline {' . $heading_fontsize . '}';
			$css .= "\n";
		}
		return $css;
	}
	public function scripts() {
		return array(
			\Joomla\CMS\Uri\Uri::base( true ) . '/plugins/sppagebuilder/jollyany/assets/js/heading.js',
		);
	}
	public function stylesheets() {
		return array(
			\Joomla\CMS\Uri\Uri::base( true ) . '/plugins/sppagebuilder/jollyany/assets/css/heading.css',
		);
	}
}
