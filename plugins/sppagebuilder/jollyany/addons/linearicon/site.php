...
class SppagebuilderAddonLinearicon extends SppagebuilderAddons {
	...
	public function render() {
		...
		$class .= (isset($settings->hover_effect) && $settings->hover_effect) ? ' sppb-linearicon-hover-effect-' . $settings->hover_effect : '';
		...
		if($name) {
			$output   = '<div class="sppb-linearicon ' . $class . '">';
			if (!empty($link)) {
				$output .= '<a ' . $target . ' href="'.$link.'">';
			}
			$output  .= '<span class="sppb-linearicon-inner">';
	...
	public function css() {
		...
		$css = '';
		if($icon_style) {
			$css .= $addon_id . ' .sppb-linearicon-inner {';
			$css .= $icon_style;
			$css .= "\n" . '}' . "\n"	;
		}

		if($font_size) {
			$css .= $addon_id . ' .sppb-linearicon-inner i {';
			$css .= $font_size;
			$css .= "\n" . '}' . "\n"	;
		}
		...
		// Hover
		if($icon_style_hover) {
			$css .= $addon_id . ' .sppb-linearicon-inner:hover {';
	...
	public static function getTemplate() {
		...
		#>
		<style type="text/css">
		#sppb-addon-{{ data.id }} .sppb-linearicon-inner {
			margin: {{ margin }};
		...
		#sppb-addon-{{ data.id }} .sppb-linearicon-inner i{
		...
		#sppb-addon-{{ data.id }} .sppb-linearicon-inner:hover {
		...
		if(data.name){
		#>
			<div class="sppb-linearicon {{ data.alignment }} {{ data.class }}">
				...
				#>
				<span class="sppb-linearicon-inner">
					<i class="{{ icon_name }}"></i>
				</span>
				<# if(!_.isEmpty(data.link)){ #>
					</a>
