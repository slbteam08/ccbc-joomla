...
class SppagebuilderAddonUKImage extends SppagebuilderAddons {
	...
	public function render() {
		...
		$image_dark             = ( isset( $settings->image_dark ) && $settings->image_dark ) ? $settings->image_dark : '';
		$image_src              = isset( $image->src ) ? $image->src : $image;
		$image_src_dark         = isset( $image_dark->src ) ? $image_dark->src : $image_dark;
        $image_webp_enable      = ( isset( $settings->image_webp_enable )) ? $settings->image_webp_enable : 0;
        $dark_mode              = ( isset( $settings->dark_mode )) ? $settings->dark_mode : 0;
        $image_webp             = ( isset( $settings->image_webp ) && $settings->image_webp ) ? $settings->image_webp : '';
        $image_webp_dark        = ( isset( $settings->image_webp_dark ) && $settings->image_webp_dark ) ? $settings->image_webp_dark : '';
        $image_webp_src         = isset( $image_webp->src ) ? $image_webp->src : $image_webp;
        $image_webp_src_dark    = isset( $image_webp_dark->src ) ? $image_webp_dark->src : $image_webp_dark;
        ...
        if ($dark_mode) {
            $image_dark_properties   =   false;
            if ( strpos( $image_src_dark, 'http://' ) !== false || strpos( $image_src_dark, 'https://' ) !== false ) {
                $image_dark_properties   =   getimagesize($image_src_dark);
            } elseif ( $image_src_dark ) {
                if (file_exists(JPATH_BASE . '/' . $image_src_dark)) $image_dark_properties   =   getimagesize(JPATH_BASE . '/' . $image_src_dark);
                $image_src_dark = JURI::base( true ) . '/' . $image_src_dark;
            }
            $data_image_src_dark     =   $image_src_dark;

            if ($image_webp_enable && $image_webp_src_dark) {
                if ( $image_webp_src_dark && (!strpos( $image_webp_src_dark, 'http://' ) !== false && !strpos( $image_webp_src_dark, 'https://' ) !== false )) {
                    $image_webp_src_dark = JURI::base( true ) . '/' . $image_webp_src_dark;
                }
                $data_image_src_dark     =   $image_webp_src_dark;
            }

            if ($image_dark_properties && is_array($image_dark_properties) && count($image_dark_properties) > 2) {
                $data_image_src_dark = 'data-src="' . $data_image_src_dark . '" data-origin="'.$image_src_dark.'" data-type="'.$image_dark_properties['mime'].'" data-width="' . $image_dark_properties[0] . '" data-height="' . $image_dark_properties[1] . '" uk-img';
            } else {
                $data_image_src_dark = 'src="' . $data_image_src_dark . '"';
            }
        }
        ...
        if (isset($title_link->src)) $title_link = $title_link->src;
		...
		if ( $image_src ) {
            ...
            $output .= '<picture'.($dark_mode ? ' class="ukimage-inner"' : '').'>';
			...
			$output .= '</picture>';
            if ($dark_mode) {
                $output .= '<picture class="ukimage-dark">';
                $output .= '<img class="el-image-dark' . $image_svg_color . $image_transition . $image_styles . $media_blend_mode . '" ' . $data_image_src_dark . ' alt="' . str_replace( '"', '', $alt_text ) . '"' . $image_svg_inline_cls . '>';
                $output .= '</picture>';
            }
