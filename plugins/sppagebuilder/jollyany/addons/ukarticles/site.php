...
class SppagebuilderAddonUKArticles extends SppagebuilderAddons{
	...
	public function render(){
        ...
        //EasyBlog
        $ebcatid        = (isset($settings->ebcatid) && $settings->ebcatid) ? $settings->ebcatid : 0;
        $ebtagids       = (isset($settings->ebtagids) && $settings->ebtagids) ? $settings->ebtagids : array();
        $eb_post_type   = (isset($settings->eb_post_type) && $settings->eb_post_type) ? $settings->eb_post_type : null;
        ...
        $eb_link_catid 	= (isset($settings->eb_link_catid)) ? $settings->eb_link_catid : 0;
		...
		} elseif ($resource == 'easyblog') {
            //Easyblog
            $ebfile = JPATH_ADMINISTRATOR . '/components/com_easyblog/includes/easyblog.php';
            if (!file_exists($ebfile)) {
                return '<p class="alert alert-danger">' . JText::_('EasyBlog component is not found.') . '</p>';
            } else {
                require_once($ebfile);

                $items = EasyBlog::getPosts([
                    'type' => $eb_post_type,
                    'sort' => $ordering,
                    'count' => $limit,
                    'includeCats' => $ebcatid,
                    'includeSubCats' => $include_subcat,
                    'includeTags' => $ebtagids
                ]);
            }
        } else {
		...
		if(count((array) $items)) {
            ...
            if($link_articles) {
                ...
                } elseif (!empty($eb_link_catid)) {
                    $output  .= '<a href="' . EBR::_('index.php?option=com_easyblog&view=categories&id=' . $eb_link_catid . '&layout=listings') . '" id="btn-' . $this->addon->id . '" class="sppb-btn' . $all_articles_btn_class . '">' . $all_articles_btn_text . '</a>';
                } else {
                    if(!empty($link_catid)){
	...
	private function renderArticle($items, $pre_val = '') {
        ...
        $card_divider_style   = (isset($settings->{$pre_val.'card_divider_style'})) && $settings->{$pre_val.'card_divider_style'} ? ' '. $settings->{$pre_val.'card_divider_style'} : '';
        ...
        $button_class   = (isset($settings->{$pre_val.'button_type'}) && $settings->{$pre_val.'button_type'}) ? ' uk-button-' . $settings->{$pre_val.'button_type'} : ' uk-button-default';
        if ($show_readmore) {
            $button_class .= (isset($settings->{$pre_val.'button_size'}) && $settings->{$pre_val.'button_size'}) ? ' uk-button-' . $settings->{$pre_val.'button_size'} : '';
        ...
        $output	.= '<div class="uk-child-width-1-'.$column_xl.'@xl uk-child-width-1-'.$column_l.'@l uk-child-width-1-'.$column_m.'@m uk-child-width-1-'.$column_s.'@s uk-child-width-1-'.$column_xs.$card_gutter.($use_slider ? ' uk-slider-items': '').($card_divider ? ' uk-grid-divider'.$card_divider_style : '').'" data-uk-grid>';

        foreach ($items as $key => $item) {
            $params = json_decode($item->attribs, true);
            $output .= '<div>';
            $output .= '<div class="sppb-addon-article uk-article uk-card'.$card_style.$card_size_cls.$card_border_radius.(!$hide_thumbnail && ($image_position == 'left' || $image_position == 'right') ? ' uk-grid-collapse' : '').($image_position == 'bottom' ? ' uk-grid-small' : '').'"'.(!$hide_thumbnail && ($image_position == 'left' || $image_position == 'right' || $image_position == 'bottom') ? ' data-uk-grid' : '').'>';

            $tmp_res    =   $resource;
            if ($resource == 'easyblog') {
                $item->link         =   $item->getPermalink();
                $item->publish_up   =   $item->getPublishDate()->toSql();
                $item->introtext    =   $item->getIntro(false, false);
            }
            if(!$hide_thumbnail) {
                $image = '';
                if ($resource == 'k2') {
                ...
                if ($resource == 'easyblog') {
                    if ($item->posttype == 'video' && isset($item->video_src) && $item->video_src) {
                        $output .= '<div class="entry-video o-aspect-ratio" style="--aspect-ratio: 16/9;">';
                        $output .= '<object class="" style="width:100%;height:100%;" data="' . $item->video_src . '">';
                    ...
                    else if ($item->posttype == 'audio' && isset($item->audio_embed) && $item->audio_embed) {
                        $output .= '<div class="entry-audio embed-responsive embed-responsive-16by9">';
                        $output .= $item->audio_embed;
                        $output .= '</div>';
                    }
                    else if ($item->posttype == 'link' && isset($item->link_url) && $item->link_url) {
                        ...
                        //Lazyload image
                        $image  =   $item->getImage();
//                        $output .= '<a class="sppb-article-img-wrap" href="' . $item->getPermalink() . '" itemprop="url"><img class="sppb-img-responsive sppb-element-lazy" src="' . $item->getImage() . '" alt="' . $item->getTitle() .'" itemprop="thumbnailUrl" data-large="' . $item->getImage() .'" loading="lazy"></a>';
                    }
                }
                if($resource != 'k2' && $resource != 'easyblog' && $item->post_format=='gallery') {
                ...
                } elseif( $resource != 'k2' && $resource != 'easyblog' &&  $item->post_format == 'video' && isset($item->video_src) && $item->video_src ) {
                    ...
                    $output .= '</object>';
                    $output .= '</div>';
                } elseif($resource != 'k2' && $resource != 'easyblog' && $item->post_format == 'audio' && isset($item->audio_embed) && $item->audio_embed) {
                    $output .= '<div class="entry-audio embed-responsive embed-responsive-16by9">';
                    $output .= $item->audio_embed;
                    $output .= '</div>';
                } elseif($resource != 'k2' && $resource != 'easyblog' && $item->post_format == 'link' && isset($item->link_url) && $item->link_url) {
                    $output .= '<div class="entry-link">';
                    $output .= '<a target="_blank" rel="noopener noreferrer" href="' . $item->link_url .'"><h4>' . $item->link_title .'</h4></a>';
                    $output .= '</div>';
                } else {
                    if (empty($image) && $resource != 'k2' && $resource != 'easyblog') {
                        jimport('jollyany.framework.article');
                        $astroidArticle = new JollyanyFrameworkArticle($item, true);
                    ...
                    if(isset($image) && $image) {
                        ...
                        if ($image_position == 'bottom') {
                            $output .=  '<div class="uk-card-media-'.$image_position.' uk-cover-container uk-width-1-1'.($image_position == 'bottom' ? ' uk-flex-last@m' : '').'">';
                        }
            ...
            if($show_author || $show_category || $show_date) {
                ...
                if($show_category) {
                    if ($resource == 'k2') {
                        ...
                        foreach ($item->getCategories() as $category) {
                            $output .= '<span class="sppb-meta-category"><a href="' . $category->getPermalink() . '" itemprop="genre">' . $category->getTitle() . '</a></span>';
                        }
                    } else {
                ...
                if($show_author) {
                    if ($resource == 'easyblog') {
                        $output .= '<span class="sppb-meta-author" itemprop="name">' . $item->getAuthorName() . '</span>';
                    } else {
            ...
            if($show_intro) {
                $output .= '<div class="sppb-article-introtext uk-margin">'. mb_substr(strip_tags($item->introtext), 0, $intro_limit, 'UTF-8') . (strlen(strip_tags($item->introtext)) > $intro_limit ? '...' : '') .'</div>';
            }
            ...
            if($show_readmore) {
                if (isset($settings->button_type) && $settings->button_type != 'link') {
                    $button_class = ' uk-button ' . $button_class ;
                }
	...
	private function renderCSS($type = '') {
        ...
        $overlay_type = (isset($settings->{$pre_val.'overlay_type'}) && $settings->{$pre_val.'overlay_type'}) ? $settings->{$pre_val.'overlay_type'} : '';
        $media_overlay_style = '';
        if($overlay_type == 'color'){
            $media_overlay_style = (isset($settings->{$pre_val.'overlay_color'}) && $settings->{$pre_val.'overlay_color'})  ? 'background-color: '.$settings->{$pre_val.'overlay_color'}.';' : '';
        } elseif ($overlay_type=='gradient'){
            $overlay_gradient = isset( $settings->{$pre_val.'overlay_gradient'} ) && $settings->{$pre_val.'overlay_gradient'}  ? $settings->{$pre_val.'overlay_gradient'} : '';
        ...
        if (!isset($settings->{$pre_val.'overlay_type'}) && !$media_overlay_style) {
            $media_overlay_style = (isset($settings->{$pre_val.'overlay_color'}) && $settings->{$pre_val.'overlay_color'})  ? 'background-color: '.$settings->{$pre_val.'overlay_color'}.';' : '';
        }
        ...
        $title_style_sm     .=  (isset($settings->{$pre_val.'title_fontsize_sm'}) && $settings->{$pre_val.'title_fontsize_sm'}) ? 'font-size:'.$settings->{$pre_val.'title_fontsize_sm'} . 'px;' : '';
        $title_style_xs     .=  (isset($settings->{$pre_val.'title_fontsize_xs'}) && $settings->{$pre_val.'title_fontsize_xs'}) ? 'font-size:'.$settings->{$pre_val.'title_fontsize_xs'} . 'px;' : '';
        ...
        $title_style_sm     .=  (isset($settings->{$pre_val.'title_lineheight_sm'}) && $settings->{$pre_val.'title_lineheight_sm'}) ? 'line-height:'.$settings->{$pre_val.'title_lineheight_sm'} . 'px;' : '';
        $title_style_xs     .=  (isset($settings->{$pre_val.'title_lineheight_xs'}) && $settings->{$pre_val.'title_lineheight_xs'}) ? 'line-height:'.$settings->{$pre_val.'title_lineheight_xs'} . 'px;' : '';
        ...
        $title_style_sm     .=  (isset($settings->{$pre_val.'title_margin_top_sm'}) && $settings->{$pre_val.'title_margin_top_sm'}) ? 'margin-top:'.$settings->{$pre_val.'title_margin_top_sm'} . 'px;' : '';
        $title_style_xs     .=  (isset($settings->{$pre_val.'title_margin_top_xs'}) && $settings->{$pre_val.'title_margin_top_xs'}) ? 'margin-top:'.$settings->{$pre_val.'title_margin_top_xs'} . 'px;' : '';
        ...
        $title_style_sm     .=  (isset($settings->{$pre_val.'title_margin_bottom_sm'}) && $settings->{$pre_val.'title_margin_bottom_sm'}) ? 'margin-bottom:'.$settings->{$pre_val.'title_margin_bottom_sm'} . 'px;' : '';
        $title_style_xs     .=  (isset($settings->{$pre_val.'title_margin_bottom_xs'}) && $settings->{$pre_val.'title_margin_bottom_xs'}) ? 'margin-bottom:'.$settings->{$pre_val.'title_margin_bottom_xs'} . 'px;' : '';
        ...
        $style .= (isset($settings->{$pre_val.'text_margin_top'}) && $settings->{$pre_val.'text_margin_top'}) ? "margin-top: " . $settings->{$pre_val.'text_margin_top'} . "px !important;" : "";
        $style .= (isset($settings->{$pre_val.'text_margin_bottom'}) && $settings->{$pre_val.'text_margin_bottom'}) ? "margin-bottom: " . $settings->{$pre_val.'text_margin_bottom'} . "px !important;" : "";
        ...
        $card_size 		= (isset($settings->{$pre_val.'card_size'}) && $settings->{$pre_val.'card_size'}) ? $settings->{$pre_val.'card_size'} : '';
        if (isset($settings->{$pre_val.'card_size_padding'}->md)) $settings->{$pre_val.'card_size_padding'} = $settings->{$pre_val.'card_size_padding'}->md;
        $card_size_padding = (isset($settings->{$pre_val.'card_size_padding'}) && $settings->{$pre_val.'card_size_padding'}) ? 'padding: ' . $settings->{$pre_val.'card_size_padding'} . ';' : '';
        $card_size_padding_sm = (isset($settings->{$pre_val.'card_size_padding_sm'}) && $settings->{$pre_val.'card_size_padding_sm'}) ? 'padding: ' . $settings->{$pre_val.'card_size_padding_sm'} . ';' : '';
        $card_size_padding_xs = (isset($settings->{$pre_val.'card_size_padding_xs'}) && $settings->{$pre_val.'card_size_padding_xs'}) ? 'padding: ' . $settings->{$pre_val.'card_size_padding_xs'} . ';' : '';

        if(!empty($meta_style)){
            $css .= $addon_id . ' .sppb-article-meta { ' . $meta_style . ' }';
        }

        if($card_size === 'custom' && $card_size_padding){
            $css .= $addon_id . ' .uk-card-custom .uk-card-body { ' . $card_size_padding . ' }';
        }
        ...
        if($card_size === 'custom' && $card_size_padding_sm){
            $css .= $addon_id . ' .uk-card-custom .uk-card-body { ' . $card_size_padding_sm . ' }';
        }
        if($title_style_sm){
            $css .= $addon_id . ' .uk-title {';
            $css .= $title_style_sm;
        ...
        if($card_size === 'custom' && $card_size_padding_xs){
            $css .= $addon_id . ' .uk-card-custom .uk-card-body { ' . $card_size_padding_xs . ' }';
        }
        if($title_style_xs){
            $css .= $addon_id . ' .uk-title {';
            $css .= $title_style_xs;
