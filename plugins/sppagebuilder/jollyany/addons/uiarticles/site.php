...
class SppagebuilderAddonUIArticles extends SppagebuilderAddons{
	...
	public function render(){
		...
		if ($resource == 'k2') {
			if ($isk2installed == 0) {
				$output .= '<p class="alert alert-danger">' . \Joomla\CMS\Language\Text::_('COM_SPPAGEBUILDER_ADDON_ARTICLE_ERORR_K2_NOTINSTALLED') . '</p>';
				return $output;
			} elseif(!file_exists($k2helper)) {
				$output .= '<p class="alert alert-danger">' . \Joomla\CMS\Language\Text::_('COM_SPPAGEBUILDER_ADDON_K2_HELPER_FILE_MISSING') . '</p>';
				return $output;
			} else {
		...
		if (!count($items)) {
			$output .= '<p class="alert alert-warning">' . \Joomla\CMS\Language\Text::_('COM_SPPAGEBUILDER_NO_ITEMS_FOUND') . '</p>';
			return $output;
		}
		...
		if(count((array) $items)) {
            ...
            if($link_articles) {
                ...
                if ($resource == 'k2') {
                    ...
                    if(!empty($link_catid)){
                        $output  .= '<a href="' . Route::_(RouteHelper::getCategoryRoute($link_catid)) . '" id="btn-' . $this->addon->id . '" class="sppb-btn' . $all_articles_btn_class . '">' . $all_articles_btn_text . '</a>';
                    }
                }
            }
			$output  .= '</div>';
	...
	private function renderArticle($items, $pre_val = '') {
        ...
        foreach ($items as $key => $item) {
            ...
            if(!$hide_thumbnail) {
                ...
                if($resource != 'k2' && $item->post_format=='gallery') {
                    if(count((array) $item->imagegallery->images)) {
                        ...
                        $output	.= '</div>';
                        $output	.= '<a class="left sppb-carousel-control" role="button" data-slide="prev" aria-label="'.\Joomla\CMS\Language\Text::_('COM_SPPAGEBUILDER_ARIA_PREVIOUS').'"><i class="fa fa-angle-left" aria-hidden="true"></i></a>';
                        $output	.= '<a class="right sppb-carousel-control" role="button" data-slide="next" aria-label="'.\Joomla\CMS\Language\Text::_('COM_SPPAGEBUILDER_ARIA_NEXT').'"><i class="fa fa-angle-right" aria-hidden="true"></i></a>';
                        $output .= '</div>';
            ...
            if ($show_event && $show_event_date && isset($params['jollyany_event_start']) && $params['jollyany_event_start']) {
                $output .= '<span class="uk-badge">' . HTMLHelper::_('date', strtotime($params['jollyany_event_start']), 'DATE_FORMAT_LC1') . '</span>';
            }
            if ($show_course) {
                jimport('jollyany.framework.course');
                if ($show_course_lecture_count) {
                    $courses    =   \Jollyany\Helper\Course::getData($item->id);
                    ...
                    if (count($courses)) {
                        ...
                        $output .= '<span class="uk-badge">' . $lectures . ' ' . \Joomla\CMS\Language\Text::_('JOLLYANY_COURSE_LECTURES') . '</span>';
                    }
                }
            }
            ...
            if($show_author || $show_category || $show_date) {
                ...
                if($show_author) {
                    ...
                    $output .= '<span class="sppb-meta-author" itemprop="name">' . \Joomla\CMS\Language\Text::sprintf('JOLLYANY_BY_AUTHOR', $author) . '</span>';
                }
            ...
            if ($show_event) {
                ...
                if ($show_event_spot && isset($params['jollyany_event_spot']) && $params['jollyany_event_spot']) {
                    $output .= '<div class="uk-grid-small" uk-grid>';
                    $output .= '<div class="uk-width-auto"><span uk-icon="icon: users; ratio: 0.8"></span></div><div class="uk-width-expand">'.\Joomla\CMS\Language\Text::sprintf('JOLLYANY_EVENT_SPOT_TEXT', $params['jollyany_event_spot']).'</div>';
                    $output .= '</div>';
                }
	...
	public function css() {
		...
		$css_path = new LayoutHelper('addon.css.button', $layout_path);
...
//            $jollyany_css_path = new LayoutHelper('addon.css.button', $layout_path);
