...
class JFormFieldNRChainedFields extends NRFormField
{
	...
	private function getValue()
	{
		...
		if (!$this->choices = \Tassos\Framework\Helpers\ChainedFields::loadCSV($this->dataset, $this->get('data_source', 'custom'), $this->separator, $this->id . '_', $this->name))
		{
			return null;
		}
    ...
    public function onAjax($options)
    {
		...
		// get all CSV data
		if (!$this->choices = \Tassos\Framework\Helpers\ChainedFields::loadCSV($csv, $data_source, $this->separator, $this->id . '_', $this->name))
		{
            echo json_encode([
				'error' => true,
				'response' => Text::_('NR_CANNOT_PROCESS_REQUEST')
	...
	private function getData()
	{
		...
		if (!$choices = \Tassos\Framework\Helpers\ChainedFields::loadCSV($this->dataset, $this->get('data_source', 'csv_file'), $this->separator, $this->id . '_', $this->name))
		{
			return [];
		}
