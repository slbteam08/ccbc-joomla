...
class JFormFieldGeoDBChecker extends FormField
{
	...
	public function renderField($options = [])
	{
        ...
        if (!\Tassos\Framework\Extension::pluginIsEnabled('tgeoip'))
        {
        ...
        if (!\Tassos\Framework\Extension::geoPluginNeedsUpdate())
        {
            return;
        }
    ...
    private function disabledPluginWarning()
    {
        return '<div class="alert alert-warning geo-db-checker" style="margin-top: 0; margin-bottom: 0;">' .
            ...
            '<p>' . Text::sprintf('NR_GEO_PLUGIN_DISABLED_DESC', '<a href="' . Route::_('index.php?option=com_plugins&view=plugins&filter[search]=System - Tassos.gr GeoIP Plugin') . '" target="_blank">', '</a>') . '</p>' .
        '</div>';
    }
