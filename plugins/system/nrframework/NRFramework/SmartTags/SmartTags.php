...
class SmartTags
{
	...
	/**
	 * List of excluded files within the Tassos\Framework\SmartTags namespace
	 * 
	 * @var  array
	 */
	 ...
	 * @param   Factory  $factory   Tassos Factory
	 */
	public function __construct($opts = [], $factory = null)
	{
		...
		// This allows registering custom Smart Tags and ensures only those are executed when needed
		if (!isset($opts['skipFrameworkTags']))
		{
			// register Tassos Smart Tags
			$this->register('\Tassos\Framework\SmartTags', dirname(__DIR__) . '/SmartTags');
		}
	}
	...
	private function replaceSmartTagsInContent(&$content, $foundSmartTags, $mixContent)
	{
		...
		foreach ($foundSmartTags as $tag)
		{
			...
			// Edge case: If the --layout option is provided without a value, $layout will be a boolean (true).
			// This can happen when the shortcode is used like {shortcode --layout} instead of {shortcode --layout=somevalue}.
			// To handle this, we convert $layout to an empty string so it doesn't break the layout logic below.
			if (is_bool($layout) && $layout === true)
			{
				$layout = '';
			}
		...
		foreach ($tag_value_pairs as $tag => $value)
		{
			...
			$value = (is_null($value) || $value == '' || (is_array($value) && empty($value))) && ($this->prepareValue || $mixContent) ? '' : $value;
	 ...
	 * 
	 * The GROUP part is required and must be pointing to \Tassos\Framework\SmartTags\GROUP file which must declare a class with the name GROUP.
	 * Eg: The shortcode {customer} will try to find a class with the name Customer in the \Tassos\Framework\SmartTags\Customer namespace.
	 * 
	 * The NAME part represents the name of the method in the called class. 
	 * For example, the shortcode {customer.name} will call the getName() method in the \Tassos\Framework\SmartTags\Customer class.
	 * 
	 * If the NAME part is ommitted or is invalid, Smart Tags fallbacks to a method with the same name as the class.
	 * For example, the shortcode {customer} will call the getCustomer() method in the \Tassos\Framework\SmartTags\Customer class.
	 *
	 * @param  string $text
	 *
	 * @return array
	...
	{	
		...
		// Regex to match --option or --option=value
		preg_match_all('/--([a-zA-Z0-9_-]+?)(?:=(.+?))?(?=\s+--|$|\s*$)/', trim($text), $matches, PREG_SET_ORDER);

		foreach ($matches as $match)
		{
			$optionName = strtolower($match[1]);
			$optionValue = isset($match[2]) ? trim($match[2]) : true;
			// Convert boolean strings ("true"/"false") to real boolean values
	...
	public function modifierShortNumber($modifierValue, &$subject)
	{
		if ($modifierValue)
		{
			$subject = \Tassos\Framework\Helpers\Number::toShortFormat($subject);
		}
	}
	 ...
	 * 
	 * Syntax: {shortcode --htmlSpecialChars=true}
	 *
	 * @param	Mixed	$modifierValue	The value of the modifier provided by the user
	 * @param	Mixed	$subject		The subject where we replace the Smart Tag
	 * 
