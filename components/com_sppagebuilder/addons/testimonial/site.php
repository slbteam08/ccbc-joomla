...
class SppagebuilderAddonTestimonial extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		if ($designation_position == 'top') {
			...
			$output .= $avatar_img ? '<img src="' . ($placeholder ? $placeholder : $avatar_img) . '" class="' . $avatar_shape . ' sppb-addon-testimonial-avatar' . ($placeholder ? ' sppb-element-lazy' : '') . '" alt="' . $alt_text . '" ' . ($placeholder ? 'data-large="' . $avatar_img . '"' : '') . ' ' . ($avatar_width ? 'width="' . $avatar_width . '"' : '') . ' ' . ($avatar_height ? 'height="' . $avatar_height . '"' : '') . ' loading="lazy">' : '';
			$output .= '<span>';
	...
	public function css()
	{
		...
		$transformCss = $cssHelper->generateTransformStyle('.sppb-addon-testimonial', $settings, 'transform');
	...
	public static function getTemplate()
	{
		...
		$output .= $lodash->generateTransformCss('.sppb-addon-testimonial', 'data.transform');
					...
					<# if(data.designation_position == "top") { #>
						<div class="sppb-testimonial-top-content sppb-addon-testimonial-footer">
							...
							<# if (avatar.src && avatar.src.indexOf("https://") == -1 && avatar.src.indexOf("http://") == -1) { #>
								<img class="{{ avatar_shape }} sppb-addon-testimonial-avatar" src=\'{{ pagebuilder_base + avatar.src }}\' width="{{ data.avatar_width }}" height="{{ data.avatar_width }}" alt="{{ data?.avatar?.alt ?? data?.name }}">
							<# } else if(avatar){ #>
								<img class="{{ avatar_shape }} sppb-addon-testimonial-avatar" src=\'{{ avatar.src }}\' width="{{ data.avatar_width }}" height="{{ data.avatar_width }}" alt="{{ data?.avatar?.alt ?? data?.name }}">
							<# } #>
							<span>
