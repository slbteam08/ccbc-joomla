...
class SppagebuilderAddonAlert extends SppagebuilderAddons
{
	...
	public function render()
	{
		...
		if ($text)
		{
			...
			$output .= '<div class="sppb-alert' . $type . ' sppb-fade in" role="alertdialog">';
	...
	public function css()
	{
		...
		$transformCss = $cssHelper->generateTransformStyle('.sppb-alert', $settings, 'transform');
	...
	public static function getTemplate()
	{
		...
		$output .= $lodash->generateTransformCss('.sppb-alert', 'data.transform');
		$output .= '
		</style>
		<div class="sppb-addon sppb-addon-alert {{ data.class }}">
			<# if( !_.isEmpty( data.title ) ){ #><{{ data.heading_selector }} class="sppb-addon-title sp-inline-editable-element" data-id={{data.id}} data-fieldName="title" contenteditable="true">{{{ data.title }}}</{{ data.heading_selector }}><# } #>
			<div class="sppb-addon-content">
				<div class="sppb-alert sppb-alert-{{ data.alrt_type }} sppb-fade in" role="alertdialog">
					<# if( data.close ){ #>
