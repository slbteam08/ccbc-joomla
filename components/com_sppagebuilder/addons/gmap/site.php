...
class SppagebuilderAddonGmap extends SppagebuilderAddons
{
	...
	public function scripts()
	{
		...
		if ($JoomlaVersion > 4)
		{
			Factory::getDocument()->getWebAssetManager()->registerAndUseScript('gmap-api', "https://maps.googleapis.com/maps/api/js?key=" . $gmap_api . '&libraries=marker', [], []);
		}
		else
		{
			HTMLHelper::_('script', "https://maps.googleapis.com/maps/api/js?key=" . $gmap_api . '&libraries=marker', [], []);
		}
	...
	public function css()
	{
		...
		$transformCss = $cssHelper->generateTransformStyle('.sppb-addon-gmap-canvas', $settings, 'transform');
		$css .= $transformCss;
	...
	public static function getTemplate()
	{
		...
		$output .= $lodash->generateTransformCss('.sppb-addon-gmap-canvas', 'data.transform');
		$output .= '
